...
export async function listTasks(userId, query) {
    const { page, limit, status, priority, projectId, parentId, isFavorite, isPinned, search, sortBy, sortOrder, dueBefore, dueAfter, includeArchived } = query;
...
export async function createTask(userId, input) {
    ...
    const task = await prisma.task.create({
        data: {
            ...taskData,
            richNotes: taskData.richNotes !== undefined ? taskData.richNotes : undefined,
            userId,
...
// ---- Duplicate Task ----
export async function duplicateTask(userId, taskId) {
...
export async function reorderTasks(userId, input) {
    const updates = input.tasks.map((t) => prisma.task.update({
        where: { id: t.id },
        data: {
...
// ---- Subtasks ----
export async function createSubtask(userId, parentId, input) {
...
export async function getComments(userId, taskId) {
    const task = await prisma.task.findFirst({
        where: { id: taskId, userId, deletedAt: null },
    });
    if (!task)
...
async function awardXpForCompletion(userId, priority) {
    const xpMap = {
        NONE: 10,
        LOW: 15,
        MEDIUM: 25,
